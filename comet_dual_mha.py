"""
comet.py
Function: comet with Dual MHA
Author: aihonfeng
History:
20241219    aihongfeng  add Dual MHA
"""
import torch
...
import numpy as np
from models import TSEncoder, MultiHeadTSEncoder, DaulMultiHeadTSEncoder, ProjectionHead
...
class COMET:
    ...
    def __init__(
        ...
        self.dualmultihead = self.pat_multihead and self.tra_multihead
        self.multihead = self.pat_multihead or self.tra_multihead
        
        if self.dualmultihead:
            print('Using DaulMultiHeadTSEncoder')
            self._net = DaulMultiHeadTSEncoder(input_dims=input_dims,
            ...
            self._net = MultiHeadTSEncoder(input_dims=input_dims,
                                            output_dims=output_dims,
                                            hidden_dims=hidden_dims,
                                            depth=depth,
                                            num_heads=num_heads, 
    ...
    def fit(self, X, y, shuffle_function='trial', masks=None, factors=None, n_epochs=None, n_iters=None, verbose=True):
        ...
        if shuffle_function == 'random':
            ...
            train_loader = DataLoader(train_dataset, batch_sampler=my_sampler) # , num_workers=0, pin_memory=True
        ...
        while True:
            ...
            for x, y in train_loader:
                ...
                # initial loss
                patient_loss = 0
                patient_views_loss = 0
                self_patient_views_loss = 0
                trial_loss = 0
                trial_views_loss = 0
                self_trial_views_loss = 0
                sample_loss = 0
                observation_loss = 0
                patient_views_out1, patient_views_out2, trial_views_out1, trial_views_out2 = None, None, None, None
                ...
                if factors[0] != 0:
                    # @ahf
                    if self.multihead and self.pat_multihead:
                        # case 1: mha for both patient and trial
                        if self.dualmultihead:
                            # do augmentation and compute representation
                            patient_out1, patient_views_out1, _= self._net(x, mask=masks[0])
                            patient_out2, patient_views_out2, _ = self._net(x, mask=masks[0])

                        # case 2: mha for patient
                        elif self.pat_multihead:
                            # do augmentation and compute representation
                            patient_out1, patient_views_out1 = self._net(x, mask=masks[0])
                            patient_out2, patient_views_out2 = self._net(x, mask=masks[0])

                        # patience view loss calculation
                        patient_views_loss = 0
                        view_num = patient_views_out1.size(1)
                        for view_id in range(view_num):
                            patient_view_loss = contrastive_loss(
                                                    patient_views_out1[:, view_id, :, :],
                                                    patient_views_out2[:, view_id, :, :],
                                                    patient_contrastive_loss,
                                                    id=pid,
                                                    hierarchical=False,
                                                    factor=factors[0],
                                                )
                            patient_views_loss += patient_view_loss

                        # self patience view loss calculation

                        # the same series and same view are postives.
                        #  B x H x O x head_dim -> B*H x O x head_dim
                        # Od, hd = patient_views_out1.size(2), patient_views_out1.size(3)
                        # self_patient_views_out1 = torch.einsum('bhmo->bmo', patient_views_out1)
                        # self_patient_views_out2 = torch.einsum('bhmo->bmo', patient_views_out2)
                        # self_patient_views_loss = contrastive_loss(
                        #     self_patient_views_out1.contiguous().view(-1, Od, hd),
                        #     self_patient_views_out2.contiguous().view(-1, Od, hd),
                        ...
                        #  B x H x O x head_dim -> H x O x B*head_dim                
                        Hd, Od = patient_views_out1.size(1), patient_views_out1.size(2)
                        self_patient_views_out1 = torch.einsum('bhmo->hmbo', patient_views_out1)
                        self_patient_views_out2 = torch.einsum('bhmo->hmbo', patient_views_out2)
                        self_patient_views_loss = contrastive_loss(
                            self_patient_views_out1.contiguous().view(Hd, Od, -1),
                            self_patient_views_out2.contiguous().view(Hd, Od, -1),
                            sample_contrastive_loss,
                            hierarchical=False,
                            factor=factors[2],
                        ) 
                    else:
                        # case 3: mha for trial
                        if self.tra_multihead:
                            # do augmentation and compute representation
                            patient_out1, _ = self._net(x, mask=masks[0])
                            patient_out2, _ = self._net(x, mask=masks[0])                        
                ...
                if factors[1] != 0:
                    # @ahf 
                    if self.multihead and self.tra_multihead:
                        # case 1: mha for both patient and trial
                        if self.dualmultihead:
                            trial_out1, _, trial_views_out1 = self._net(x, mask=masks[1])
                            trial_out2, _, trial_views_out2 = self._net(x, mask=masks[1])  
                        elif self.tra_multihead:
                            trial_out1, trial_views_out1 = self._net(x, mask=masks[1])
                            trial_out2, trial_views_out2 = self._net(x, mask=masks[1])

                        # trial view loss calculation
                        trial_views_loss = 0
                        view_num = trial_views_out1.size(1)
                        for view_id in range(view_num):
                            trial_view_loss = contrastive_loss(
                                                    trial_views_out1[:, view_id, :, :],
                                                    trial_views_out2[:, view_id, :, :],
                                                    trial_contrastive_loss,
                                                    id=pid,
                                                    hierarchical=False,
                                                    factor=factors[0],
                                                )
                            trial_views_loss += trial_view_loss

                        # the same view are postives across different trials
                        #  B x H x O x head_dim -> H x O x B*head_dim                
                        Hd, Od = trial_views_out1.size(1), trial_views_out1.size(2)
                        self_trial_views_out1 = torch.einsum('bhmo->hmbo', trial_views_out1)
                        self_trial_views_out2 = torch.einsum('bhmo->hmbo', trial_views_out2)
                        self_trial_views_loss = contrastive_loss(
                            self_trial_views_out1.contiguous().view(Hd, Od, -1),
                            self_trial_views_out2.contiguous().view(Hd, Od, -1),
                            sample_contrastive_loss,
                            hierarchical=False,
                            factor=factors[2],
                        ) 
                        ...
                        if self.pat_multihead:
                            trial_out1, _ = self._net(x, mask=masks[1])
                            trial_out2, _ = self._net(x, mask=masks[1])
                        else:
                ...
                if factors[2] != 0:
                    # @ahf 
                    if self.dualmultihead:
                        sample_out1, _, _ = self._net(x, mask=masks[2])
                        sample_out2, _, _ = self._net(x, mask=masks[2])
                    elif self.multihead:
                        sample_out1, _ = self._net(x, mask=masks[2])
                        sample_out2, _ = self._net(x, mask=masks[2])
                    else:
                ...
                if factors[3] != 0:
                    # @ahf 
                    if self.dualmultihead:
                        observation_out1, _, _ = self._net(x, mask=masks[3])
                        observation_out2, _, _ = self._net(x, mask=masks[3])
                    elif self.multihead:
                        observation_out1, _ = self._net(x, mask=masks[3])
                        observation_out2, _ = self._net(x, mask=masks[3])
                    else:
                ...
                # @ahf
                if self.multihead:
                    loss = patient_loss + patient_views_loss + self_patient_views_loss + trial_loss + + trial_views_loss + self_trial_views_loss + sample_loss + observation_loss
                else:
    ...
    def eval_with_pooling(self, x, mask=None):
        ...
        # @ahf
        if self.dualmultihead:
            out, pat_views_out, tri_views_out = self.net(x, mask)
            ...
            # maxpool for out and views_out seperately and the concat
            # out = F.max_pool1d(
            #     out.transpose(1, 2),
            ...
            # pat_views_out = F.max_pool1d(
            #     pat_views_out.transpose(1, 2),
            #     kernel_size=pat_views_out.size(1),
            # ).transpose(1, 2)
            # pat_views_out = pat_views_out.squeeze(1)

            # out = torch.cat([out, views_out], dim=1)

            # tri_views_out = F.max_pool1d(
            #     tri_views_out.transpose(1, 2),
            #     kernel_size=tri_views_out.size(1),
            # ).transpose(1, 2)
            # tri_views_out = tri_views_out.squeeze(1)

            # out = torch.cat([out, tri_views_out], dim=1)
            ...
            # actually concat then maxpool is better
            # out = torch.cat([out, views_out.contiguous().view(B, Od, -1)], dim=2)
            ...
            # views_out = F.max_pool1d(
            #     views_out.transpose(1, 2),
            #     kernel_size=views_out.size(1),
            # ).transpose(1, 2)
            # views_out = views_out.squeeze(1)
