...
def load_tdbrain(train_ids, val_ids, test_ids, data_path, label_path):
    ...
    for j in range(len(filenames)):
        ...
        for trial_feature in subject_feature:
            ...
            # training set
                ...
                print(f'train_id:{j+1}, while train_label:{trial_label}')
    # reshape and shuffle
    ...
    # 打印每个集合的标签分布
    print("Training set labels:", np.unique(y_train[:, 0], return_counts=True))
    print("Validation set labels:", np.unique(y_val[:, 0], return_counts=True))
    print("Test set labels:", np.unique(y_test[:, 0], return_counts=True))
    subject_label = np.load(label_path)
    print("Unique labels in label file:", np.unique(subject_label[:, 0]))
