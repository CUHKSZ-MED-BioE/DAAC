...
class DaulMultiHeadTSEncoder(nn.Module):
    def __init__(self, input_dims, output_dims, hidden_dims=64, depth=10, mask_mode='binomial',
        ...
        self.repr_dropout = nn.Dropout(p=0.1)
        self.MHA1 = MHA(num_heads, head_dim, channel_dim)
        self.MHA2 = MHA(num_heads, head_dim, channel_dim)
    ...
    def forward(self, x, mask=None):  # input dimension : B x O x Ci
        ...
        x_views1 = self.MHA1(x) # B x H x O x head_dim 
        x_views2 = self.MHA2(x) # B x H x O x head_dim 

        x = self.repr_dropout(x)  # B x Co x O
        x_views1 = self.repr_dropout(x_views1)
        x_views2 = self.repr_dropout(x_views2)

        x = x.transpose(1, 2)  # B x O x Co
        
        return x, x_views1, x_views2