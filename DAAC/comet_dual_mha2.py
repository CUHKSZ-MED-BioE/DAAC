...
class COMET:
                ...
                if factors[0] != 0:
                    ...
                    if self.multihead and self.pat_multihead:
                        ...
                        if self.dualmultihead:
                            ...
                            patient_out1, patient_views_out1, patient_mask_views_out1 = self._net(x, mask=masks[0])
                            patient_out2, patient_views_out2, patient_mask_views_out2 = self._net(x, mask=masks[0])
                        ...
                        Hd, Od = patient_mask_views_out1.size(1), patient_mask_views_out1.size(2)
                        self_patient_mask_views_out1 = torch.einsum('bhmo->hmbo', patient_mask_views_out1)
                        self_patient_mask_views_out2 = torch.einsum('bhmo->hmbo', patient_mask_views_out2)
                        self_patient_views_loss = contrastive_loss(
                            self_patient_mask_views_out1.contiguous().view(Hd, Od, -1),
                            self_patient_mask_views_out2.contiguous().view(Hd, Od, -1),
                            sample_contrastive_loss,
                            hierarchical=False,
                            factor=factors[2],
                        ) 
                ...
                if factors[1] != 0:
                    ...
                    if self.multihead and self.tra_multihead:
                        ...
                        if self.dualmultihead:
                            ...
                            trial_out1, trial_views_out1, _ = self._net(x, mask=masks[1])
                            trial_out2, trial_views_out2, _ = self._net(x, mask=masks[1])
                        ...
                        if self.pat_multihead:
                            trial_out1, _, _ = self._net(x, mask=masks[1])
                            trial_out2, _, _ = self._net(x, mask=masks[1])
                        else:
    ...
    def eval_with_pooling(self, x, mask=None):
        ...
        if self.dualmultihead:
            ...
            out, views_out, views_mask_out = self.net(x, mask)
            # concat out and views_out
            # B x H x O x head_dim -> B x O x H*head_dim 
