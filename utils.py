...
def split_data(X_trial, sample_timestamps=256, overlapping=0.5):
    ...
    # 检查 sub_length 和 overlapping 是否兼容
    if overlapping:
        # 计算样本数量
        sample_num = (length - (1 - overlapping) * sample_timestamps) / (sample_timestamps * overlapping)
        # 如果 sample_num 不是整数，调整 length 使其兼容
        if not sample_num.is_integer():
            # 调整 length 为最接近的兼容值
            length = int((length // (sample_timestamps * overlapping)) * (sample_timestamps * overlapping) + (1 - overlapping) * sample_timestamps)
            sample_num = (length - (1 - overlapping) * sample_timestamps) / (sample_timestamps * overlapping)
        sample_num = int(sample_num)  # 转换为整数
    else:
        # 如果 length 不能被 sample_timestamps 整除，调整 length
        if length % sample_timestamps != 0:
            length = (length // sample_timestamps) * sample_timestamps
        sample_num = length // sample_timestamps  # 计算样本数量
    sample_feature_list = []
    trial_id_list = []
    trial_id = 1
